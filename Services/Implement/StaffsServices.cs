...
{
    public class StaffsServices : IStaffsServices
    {
        ...
        public StaffsServices(IDapperRepository dapperRepository, IOptions<AppSettings> options, UserManager<AppUser> userManager, IUnitOfWork unitOfWork, IAuthenticatedUserService authenticatedUser)
        {
        ...
        public async Task<IPagedResults<StaffViewModel>> GetAllAsync(PagingRequest request)
        {
            try
            {
                ...
                var lstStaffs = await _dapperRepository.QueryMultipleUsingStoreProcAsync<StaffViewModel>("uspStaffs_selectAll", p);
                var staffViewModels = lstStaffs.ToList();

                var total = staffViewModels.Count;
                lstStaffs = !string.IsNullOrEmpty(request.SortField) ? staffViewModels.OrderBy(request) : staffViewModels.OrderByDescending(x => x.Id);

                if (request.Page != null && request.PageSize.HasValue)
                {
                    lstStaffs = lstStaffs.Paging(request);
                }
                ...
                var viewModels = lstStaffs.ToList();
                if (viewModels.Count > 0)
                {
                    var queryStaffPlace = $"SELECT sp.[Id], sp.[StaffId], sp.[PlaceId], CONCAT('Khu nhà: ',p.Title, ' - ', p1.[Name],', ',CONCAT(d.[Prefix], '', d.[Name]),', ',CONCAT(w.[Prefix], '', w.[Name]),CONCAT(s.[Prefix], '', s.[Name]),pr.[Name]) AS LocationName FROM StaffPlace AS sp LEFT JOIN Places AS p ON p.Id = sp.PlaceId AND p.IsDeleted = 0 LEFT JOIN Province AS p1 ON p1.Id = p.ProvinceId LEFT JOIN District AS d ON d.Id = p.DistrictId LEFT JOIN Ward AS w ON w.WardCode = p.WardId and p.ProvinceId = w.ProvinceId and p.DistrictId = w.DistrictId LEFT JOIN Street AS s ON s.StreetCode = p.WardId and p.ProvinceId = s.ProvinceId and p.DistrictId = s.DistrictId LEFT JOIN Project AS pr ON pr.ProjectCode = p.WardId and p.ProvinceId = pr.ProvinceId and p.DistrictId = pr.DistrictId WHERE sp.StaffId IN ({string.Join(",", viewModels.Select(x => x.Id))})";
                    var staffPlace = _dapperRepository.QueryMultiple<StaffPlaceViewModel>(queryStaffPlace);
                    var listStaffPlace = staffPlace.ToList();
                    ...
                    viewModels.ForEach(x =>
                    {
                        ...
                        x.ListStaffPlace = listStaffPlace.FindAll(d => d.StaffId == x.Id);
            ...
            {
                Log.Fatal(DateTime.Now.ToString(CultureInfo.InvariantCulture) + " StaffsServices: " + ex.Message);
                throw;
            }
        }
        ...
        public async Task<IPagedResults<ItemModel>> SuggestionAsync(string searchText)
        {
            try
            {
                ...
                p.Add(Constants.CompanyId, _authenticatedUser.CompanyId);
                var lstStaffs = await _dapperRepository.QueryMultipleUsingStoreProcAsync<ItemModel>("uspStaffs_Suggestion", p);
                ...
                var itemModels = lstStaffs.ToList();
                return new PagedResults<ItemModel>
                {
                    ResponseCode = Convert.ToInt32(HttpStatusCode.OK),
                    ResponseMessage = Constants.ErrorMessageCodes.SuccessMessage,
