IF NOT EXISTS ( SELECT  1
            FROM    sys.procedures
            WHERE   name = 'uspZones_SuggestionColumn'
                    AND SCHEMA_NAME(schema_id) = 'dbo' )
    BEGIN 
        exec('CREATE PROCEDURE [dbo].uspZones_SuggestionColumn AS BEGIN SET NOCOUNT ON; END')
    END
GO
ALTER PROCEDURE [dbo].[uspZones_SuggestionColumn]
    (
	    @key NVARCHAR(100),
		@BasementId BIGINT,
		@CompanyId BIGINT
    ...
    BEGIN
		...
		WITH TempResult AS 
		(
			SELECT Id, Title AS [Value] FROM dbo.[Columns] 
                            WHERE BasementId = @BasementId AND 
                            (
                                ISNULL(@key, '') = '' OR 
