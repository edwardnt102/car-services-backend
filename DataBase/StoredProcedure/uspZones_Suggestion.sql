IF NOT EXISTS ( SELECT  1
            FROM    sys.procedures
            WHERE   name = 'uspZones_Suggestion'
                    AND SCHEMA_NAME(schema_id) = 'dbo' )
    BEGIN 
        exec('CREATE PROCEDURE [dbo].uspZones_Suggestion AS BEGIN SET NOCOUNT ON; END')
    END
GO
ALTER PROCEDURE [dbo].[uspZones_Suggestion]
    (
	    @key NVARCHAR(100),
		@CompanyId BIGINT
    )
    ...
    BEGIN
		...
		WITH TempResult AS 
		(
			SELECT Id, Title as [Value] FROM [dbo].Zones 
