    ...
    public class BrandsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllAsync([FromQuery] PagingRequest request)
        {
            try
            {
                ...
                var result = await _brandsServices.GetAllAsync(request);

                return Ok(result);
            }
            catch (Exception ex)
            {
                Log.Fatal(DateTime.Now.ToString(CultureInfo.InvariantCulture) + " BrandsController: " + ex.Message);
        ...
        public async Task<IActionResult> SuggestionAsync(string textSearch)
        {
            try
            {
                ...
                return Ok(await _brandsServices.SuggestionAsync(textSearch));
            }
            catch (Exception ex)
            {
                Log.Fatal(DateTime.Now.ToString(CultureInfo.InvariantCulture) + " BrandsController: " + ex.Message);
        ...
        [HttpPost("Save")]
        public async Task<IActionResult> SaveAsync([FromForm] Brands model, List<IFormFile> files, IFormFile logo)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                return Ok(await _brandsServices.SaveAsync(model, files, logo));
            }
            catch (Exception ex)
            {
                Log.Fatal(DateTime.Now.ToString(CultureInfo.InvariantCulture) + " BrandsController: " + ex.Message);
        ...
        public async Task<IActionResult> DeleteAsync(long id)
        {
            try
            {
                ...
                return Ok(await _brandsServices.DeleteAsync(id));
            }
            catch (Exception ex)
            {
                Log.Fatal(DateTime.Now.ToString(CultureInfo.InvariantCulture) + " BrandsController: " + ex.Message);
